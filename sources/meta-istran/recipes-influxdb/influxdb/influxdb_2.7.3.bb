...
do_install () {
    	...
    	install -m 0644 ${S}/usr/lib/influxdb/scripts/influxdb.service ${D}/usr/lib/influxdb/scripts/influxdb.service
    	install -m 0644 ${S}/usr/lib/influxdb/scripts/influxd-systemd-start.sh ${D}/usr/lib/influxdb/scripts/influxd-systemd-start.sh
    	install -m 0644 ${S}/usr/lib/influxdb/scripts/init.sh ${D}/usr/lib/influxdb/scripts/init.sh
    	install -m 0644 ${S}/usr/share/influxdb/influxdb2-upgrade.sh ${D}/usr/share/influxdb/influxdb2-upgrade.sh
    	install -m 0644 ${S}/etc/logrotate.d/influxdb ${D}/etc/logrotate.d/influxdb
    	install -m 0644 ${S}/usr/bin/influxd ${D}/usr/bin/influxd
}
