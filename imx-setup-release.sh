...
echo "    packagegroup-core-ssh-openssh \\" >> conf/local.conf
echo "    openssh-sftp-server \\" >> conf/local.conf
echo "    xrandr \\" >> conf/local.conf
echo "    xdpyinfo \\" >> conf/local.conf
...
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-custom-dmp\"" >> $BUILD_DIR/conf/bblayers.conf
